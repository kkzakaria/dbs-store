    ...
    (set, get) => ({
      ...
      addItem: (product: CartProduct, quantity: number = 1) => {
        ...
        const existingItem = items.find(
          (item) =>
            makeCartItemKey(item.product.id, item.product.variant_id) ===
              itemKey,
        );
      ...
      loadFromServer: async (userId: string) => {
        ...
        const typedData = data as unknown as CartItemQueryResult[];

        const items: CartItem[] = typedData
          ...
          .map((item) => {
            ...
            const stockQuantity = v?.stock_quantity ?? p.stock_quantity ?? 0;
            const variantOptions = v?.options && typeof v.options === "object"
      ...
      mergeServerCart: async (userId: string) => {
        ...
        const typedServerData = serverData as unknown as CartItemQueryResult[];

        const serverItems: CartItem[] = typedServerData
          .filter((item) => item.product !== null)
          .map((item) => {
            const p = item.product!;
            const v = item.variant;
        ...
        for (const serverItem of serverItems) {
          const serverKey = makeCartItemKey(
            serverItem.product.id,
            serverItem.product.variant_id,
          );
