...
export function ProductCard({
  ...
  return (
    ...
    >
      <Link href={`/products/${product.slug}`} className="flex flex-col h-full bg-white dark:bg-card border border-border/40 rounded-3xl sm:rounded-[32px] overflow-hidden transition-all duration-300 hover:border-border/80">
        ...
        >
          ...
          <div className="absolute left-5 top-5 sm:left-8 sm:top-8 flex flex-col gap-2 z-10">
            {hasDiscount && (
              <span className="px-3 py-1 text-[10px] font-bold bg-[#e6f4ea] text-[#137333] dark:bg-green-500/20 dark:text-green-400 rounded-full uppercase tracking-wider">
                Offre
              </span>
            )}
            {product.is_featured && !hasDiscount && (
              <span className="px-3 py-1 text-[10px] font-bold bg-[#e8f0fe] text-[#1967d2] dark:bg-blue-500/20 dark:text-blue-400 rounded-full uppercase tracking-wider">
                Nouveau
              </span>
            )}
            {isOutOfStock && (
              <span className="px-3 py-1 text-[10px] font-bold bg-muted/60 text-muted-foreground rounded-full uppercase tracking-wider">
                Bientôt
              </span>
            )}
          </div>
          ...
          <Button
            ...
            size="icon"
            className={cn(
              "absolute top-5 right-5 sm:top-8 sm:right-8 z-20",
        ...
        <div className="flex flex-col px-6 sm:px-8 pt-6 sm:pt-8 pb-8 flex-grow">
          <div className="space-y-2 mb-4">
            ...
            {product.category && (
              <p className="text-[11px] text-muted-foreground uppercase tracking-widest leading-none">
          ...
          <div className="mt-auto space-y-4">
            ...
            <div className="flex items-center gap-3">
              <span className="text-lg font-medium text-foreground">
                {hasDiscount ? "À partir de " : ""}{formatPrice(product.price)}
              </span>
            ...
            {hasDiscount && (
              <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-[#e6f4ea] dark:bg-green-500/10 text-[#137333] dark:text-green-400 text-[11px] font-bold">
            ...
            <div className="pt-2">
              <span
                className={cn(
                  "inline-flex items-center text-[#1a73e8] dark:text-[#8ab4f8] font-bold text-sm tracking-wide group-hover:underline",
                  isOutOfStock && "text-muted-foreground"
                )}
              >
                {isOutOfStock ? "En savoir plus" : "Acheter"}
